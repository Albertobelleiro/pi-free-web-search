...
const search = await runSearch(cwd, { query, numResults: 3, includeContent: false, mode: "headless" });
...
const content = await fetchContent(cwd, search.results[0].url, "headless");
